...
fs=s/(s+100);%inverse of pseudo integrater

fs=tf(1);

w=[logspace(-2,3,30)]; %frequency points
...
while g_max-g_min > g_tol
    ...
    Wfgamma =inv(gamma)*Wf;
        ...
        for q=1:nq% gridding the theta
            for j=1:2*num
                if j<=num
                    phiGq(j,:)= [squeeze(phif(j,1,:).*Nf(1,i,:))'-exp(1i*2*pi*q/nq)/cos(pi/nq)*squeeze(Wfgamma(2,1,:))'.*squeeze(phif(j,1,:).*Nf(1,i,:))'];% ST SS SU
                else
                    phiGq(j,:)= [squeeze(phif(j-num,1,:).*fsf.*Mf(1,i,:))'];%=
                end
            end
            A1=-real(transpose(phiGq));
            h=size(A1);
...
% Matlab Solution
Wperf=W(1,1);Wnoise=W(2,1);
Parrayg = frd(Parray,logspace(-1,3,60)); 
[P,Info] = ucover(Parrayg,Pnom,4,'InputMult');
P.u = 'u';   P.y = 'yp';
...
[C,CL,mubound]=dksyn(G,1,1);
mubound
SS=norm(minreal(W(1,1)*feedback(1,C*Parray)),inf)
ST=norm(minreal(W(2,1)*feedback(C*Parray,1)),inf)



figure,hh=stepplot(minreal(feedback(Parray*K,1)),10/desBW);
P = timeoptions;
P.Title.String=[];
