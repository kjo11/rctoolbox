...
\subsection{Linearly Parameterize Controllers}
...
\subsection{Smith Predictor Controllers}
A Smith Predictor controller has the form shown in Fig. \ref{fig:SP_layout}, where $P_i(s)$ is the plant model, $P_n(s)$ is the nominal plant model, $G_n(s)$ is the nominal, delay-free plant model and $C(s)$ is the controller. The open-loop transfer function $L_i(s) = \left[ P_i(s) + H(s) \right] C(s)$, where $H(s)=G_n(s)-P_n(s)$. $C(s)$ is a linearly parameterized controller, as described above.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/smith_mimo.pdf}
...
\item[\texttt{ConStruc}] is either \texttt{'LP'} for a linearly parameterized controller (default) or \texttt{'SP'} for a Smith Predictor controller.

\item[\texttt{ConOpt}] for a Smith Predictor controller gives $H(s)=G_n(s)-P_n(s)$.
...
\subsection{Example 6} A Smith Predictor, PID controller structure
...
\subsubsection{Smith Predictor controller design with $H_\infty$ performance}
Consider the family of plants:

$$
P_i(s)=\frac{e^{-\tau_i s}}{(5 s + 1)(10 s + 1)}
$$

where $\tau_i$ belongs to the set $\{ 4.5, 5, 5.5 \}$.

Design a PID controller  to optimize  the robust performance
$$ \| |W_1 S| +  |W_2 T| \|_\infty < \gamma $$
with
$$W_1(s) = \frac{2}{(30 s + 1)^2}$$
and
$$W_2(s) = \frac{-s^2-2s}{s^2+2s+1}$$

We choose the following $L_d(s)$ that contains the unstable pole of the plant model and meets the Nyquist stability criterion.
$$L_d(s)=\frac{0.1}{s}$$.
