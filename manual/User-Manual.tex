...
\subsection{Linearly Parameterized Controllers}
...
\subsection{State Space Controllers}
...
In the toolbox, the eigenvalues of the A matrix (equivalent to the poles of the system) and either the B or C matrix are specified. $\rho$ represents the entries of the B or C matrix (whichever was not specified) and of the D matrix.
...
\subsection{SISO Rational Controllers}
...
$\phi$ is a vector of Laguerre or generalized basis functions, described above, and $f(s)$ is a fixed part of the controller. $\rho_x$ and $\rho_y$ are vectors of controller parameters.
...
\item[\texttt{ConStruc}] is either \texttt{'LP'} for a linearly parameterized controller (default), \texttt{'TF'} for a transfer function (rational) controller, or \texttt{'SS'} for a state-space controller.
...
Continuous-time transfer function.
  ...
  \subsection{Example 7} A continuous time PID controller defined in state space
...
\subsection{Example 8} A discrete time Laguerre controller defined in state space with the B matrix given
...
\item{\texttt{`Hinf'}} In this method the distance between $L=GK$ and $L_d$ is minimized under 
             some $H_\infty$ constraints on the weighted closed-loop sensitivity functions. For rational controllers, $L_d$ is not used when $\gamma$ is minimized. 
